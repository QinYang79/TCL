...
class EncoderText_glove(nn.Module):
    def __init__(self, vocab_size, word_dim, embed_size, num_layers,
                 use_bi_gru=False, no_txtnorm=False,data_name='f30k_precomp'):
        ...
        self.embed = GloveEmb(
            vocab_size,
            glove_dim=word_dim,
            glove_path=f'/home/qinyang/projects/TCL/vocab/glove_840B_{data_name}.json.pkl',
            add_rand_embed=False,
            rand_dim=word_dim,
        )
        ...
        self.cap_rnn = nn.GRU(self.embed.final_word_emb, embed_size, num_layers, batch_first=True,
...
class EncoderText(nn.Module):
    ...
    def __init__(self, vocab_size, word_dim, embed_size, num_layers,
                 use_bi_gru=False, no_txtnorm=False,word2idx=None):
...
class SGRAF(object):
    ...
    def __init__(self, opt,word2idx=None):
        ...
        opt.sim_dim=256
        opt.sgr_step=3
        opt.module_name = opt.method
        self.img_enc = EncoderImage(opt.img_dim, opt.embed_size,
                                    no_imgnorm=opt.no_imgnorm)
        if 'bert' in opt.method:
            self.txt_enc = EncoderText_BERT(opt)
            opt.module_name = opt.method.split('_')[0]
        if 'glove' in opt.method: 
            self.txt_enc = EncoderText_glove(opt.vocab_size, opt.word_dim,
                                    opt.embed_size, opt.num_layers,
                                    use_bi_gru=opt.use_bi_gru,
                                    no_txtnorm=opt.no_txtnorm,data_name=opt.data_name)
            opt.module_name = opt.method.split('_')[0]
        else:
    ...
    def forward_sim(self, img_embs, cap_embs, cap_lens, mode='sim'):
        ...
        if mode == 'sim':
            ...
            if self.active=='Exp':
                 sims, evidences, sims_tanh = torch.sigmoid(raw_sims), (
                torch.tanh(raw_sims)/self.tau).exp(), torch.tanh(raw_sims)
            elif self.active=='ReLU':
                 sims, evidences, sims_tanh = torch.sigmoid(raw_sims), (torch.tanh(raw_sims)/self.tau).relu(), torch.tanh(raw_sims)
            else:
